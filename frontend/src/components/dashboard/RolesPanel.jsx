...
import { Shield, RefreshCw, UserX, XCircle } from "lucide-react"

export default function RolesPanel({ isOpen }) {
  const [roles, setRoles] = useState([])
  const [rolesStatus, setRolesStatus] = useState("idle") // idle | loading | ready | error | forbidden
                ...
                <div className="overflow-x-auto -mx-2">
                  <table className="w-full text-left text-[13px] border-separate border-spacing-0">
                    ...
                    <tbody>
                      {roles.length === 0 ? (
                        <tr>
                          <td colSpan={2} className="px-2 py-6 text-center text-[#6B7280]">
                            Kayıt bulunamadı.
                          </td>
                        </tr>
                      ) : (
                        roles.map((role) => (
                          <tr key={role.id} className="group/row hover:bg-[#FAFBFC] transition-colors">
                            <td className="px-2 py-3 border-t border-[#F3F5F8] text-[#6B7280] font-mono text-[12.5px]">
                              {role.id}
                            </td>
